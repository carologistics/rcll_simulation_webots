...
## Table of content 
    ...
    - Launch SLAM toolbox
    - Launch NAV2 stack  
        - Navigation with single instance of robotinobase
...
## Spawn simulation environmanet
    ...
    - namespace: Its a launch configuration used to spawns the corresponding robotinobase(1/2/3), its controllers and node parameters 
    - launch_rviz: Its a launch configuration for starting the Rviz2 with predefined config file, parse 'false' when using nav2_stack 
    ...
    - Connect a joystick to your machine and operate the robot using the joystick., by default device_id associated with the joystick is 0 (no need to change for single instance of robotinobase spawn in simulation), in case of multiple instances of robotinobase spawn in simulation, change the device_id to 0 or 1 or 2 accordingly.
    - by default:
...
## Launch SLAM toolbox
    ...
    For mapping the environment, first launch the single instance of robotinobase in simulation, ensure the joystick device is connectd with correct device id (by defaul, device_id=0), then launch the SLAM toolbox by running the following command in the root of your workspace:
...
## Launch NAV2 stack
    ...
    - namespace: Its a launch configuration used to spawns the map server, amcl, nav2_stack, collision monitor and rviz2 with predefined configs for corresponding robotinobase(1/2/3)
    ...
    For autonomous navigation, first launch the multiple instances of robotinobase in simulation as described above, then launch the NAV2 stack by running the following commands in defferent terminal instances from the root of your workspace:
    ...
    It will launch the map server, amcl, nav2_stack, collision monitor and rviz2 with predefined configs for corresponding robotinobase(1/2/3). Use Rviz2 to send a goal to the corresponding robotinobase(1/2/3).
...
## Research and References
    - Omnidirectional robot kinematics and dynamics:
       ...
       Jordi Palacín; Elena Rubies; Eduard Clotet; and David Martínez; Evaluation of the Path-Tracking Accuracy of a Three-Wheeled Omnidirectional Mobile Robot Designed as aPersonal Assistant https://doi.org/10.3390/s21217216
