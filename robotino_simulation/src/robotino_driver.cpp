...
void RobotinoDriver::init(
  ...
  joint_state_pub_ = node->create_publisher<sensor_msgs::msg::JointState>(namespace_param + "/joint_states", 1);
   ...
   ir_sensor_ori_ = {
    ...
    laser_names_ = {
        "SickLaser_Rear", "SickLaser_Front", "laser_link"
    };
...
void RobotinoDriver::publish_odom(const TimeStamp &time_stamp, const double &time_diff) {
  ...
  double x = prev_odom_x_ + ((velocity[0] * cos(omega)) - (velocity[1] * sin(omega))* time_diff);
  double y = prev_odom_y_ + ((velocity[0] * sin(omega)) + (velocity[1] * cos(omega))* time_diff);
...
void RobotinoDriver::publish_ir(const TimeStamp &time_stamp) {
    for (size_t i = 0; i < ir_sensor_names_.size(); ++i) {
        geometry_msgs::msg::TransformStamped tf;
        tf.header.stamp = time_stamp;
        tf.header.frame_id = tf_prefix_ + "/base_link"; // Assuming the frame_id is fixed as "base_link"
...
void RobotinoDriver::publish_joint_state(const TimeStamp &time_stamp) {
	...
	std::vector<std::string> motor_names = {"wheel0_joint",
	"wheel1_joint",
	"wheel2_joint" };
...
std::vector<double> RobotinoDriver::kinematics() {
    ...
    double m1 = ((sqrt(3.) / 2.) * v_x) - (0.5 * v_y) - omega;
    double m2 = v_y - omega;
    double m3 = -((sqrt(3.) / 2.) * v_x) - (0.5 * v_y) - omega;
...
std::vector<double> RobotinoDriver::inverse_kinematics(const double &w0,const double &w1,const double &w2) {
	double vx = ((-w1 +w2)/ sqrt(3.0)) * WHEEL_RADIUS;
	double vy = (((2.0/3.0)* w0) - ((1.0/3.0) * w1)) * WHEEL_RADIUS;
	double const_val = 3.0*WHEEL_DISTANCE;
	double omega = (-((1.0 / const_val) * w0) - (( 1.0 / const_val) * w1) - (( 1.0 / const_val ) * w2)) * WHEEL_RADIUS;
	return {vx, vy, omega};
}
